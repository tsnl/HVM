...
pub fn as_code(mem: &Worker, comp: &Option<rb::RuleBook>, host: u64) -> String {
  ...
  fn go(ctx: &mut CtxGo, stacks: &mut Stacks, term: Lnk, depth: u32) -> String {
    ...
    println!("readback {} | {}", rt::show_lnk(term), rt::show_term(ctx.mem, rt::ask_lnk(ctx.mem, term), None));
  ...
  seen = HashSet::<Lnk>::new();
  let ctx = &mut CtxGo { mem, comp, names: &names, seen: &mut seen };
